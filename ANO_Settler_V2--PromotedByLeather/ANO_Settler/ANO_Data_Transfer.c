...
#define BYTE1(dwTemp)       ( *( (u8 *)(&dwTemp) + 1) )//beautiful--Leather
...
void ANO_DT_Data_Exchange(void) //2ms一次
{
	...
	static u8 power_cnt		=	200;//慢点发送--Leather
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		f.send_power = 0;
		ANO_DT_Send_Power((u16)(ADC_ConvertedValue*660/4096),456);
	}
/////////////////////////////////////////////////////////////////////////////////////
	else if(f.send_pid1)
	{
...
void ANO_DT_Data_Receive_Anl(u8 *data_buf,u8 num)
{
	...
	for(u8 i=0;i<(num-1);i++)
		sum += *(data_buf+i);
	if(!(sum==*(data_buf+num-1)))		return;		//判断sum，校验--Leather
	if(!(*(data_buf)==0xAA && *(data_buf+1)==0xAF))		return;		//判断帧头--Leather
