...
void Duty_6ms()
{
    ...
    IMUupdate(sensor.Gyro_deg.x,sensor.Gyro_deg.y,sensor.Gyro_deg.z, sensor.Acc_mmss.x,  sensor.Acc_mmss.y,  sensor.Acc_mmss.z);
		test_time[4] = GetSysTime_us();
		test_time[5] = test_time[4] - test_time[3];

//	OledShowData_float(imu_data.pit,1,1);
//	OledShowData_float(imu_data.rol,2,1);
//	OledShowData_float(imu_data.yaw,3,1);
...
void Duty_1000ms()//Leather
{
	MYDMA_Enable();
	if(ADC_ConvertedValue<1550)//2.5  ADC_ConvertedValue*2*3.3/4096
		GPIOA->BRR = GPIO_Pin_8;//低电压灯光提示--Leather
	else
