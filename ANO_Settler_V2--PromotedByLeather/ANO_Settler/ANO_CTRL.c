...
void CTRL_2_PID_Init()
{
	...
	arg_2_rol.ki = 0.1f   *ANO_Param.PID_rol.ki *0.001f;
	arg_2_rol.kd = 0.01f *ANO_Param.PID_rol.kd *0.001f;
	arg_2_rol.k_pre_d = 0.0f;
	arg_2_rol.k_ff = 0.0f;
	
	arg_2_pit.kp = 0.8f   *ANO_Param.PID_pit.kp *0.001f;
	arg_2_pit.ki = 0.1f   *ANO_Param.PID_pit.ki *0.001f;
	arg_2_pit.kd = 0.01f *ANO_Param.PID_pit.kd *0.001f;
	arg_2_pit.k_pre_d = 0.0f;
	arg_2_pit.k_ff = 0.0f;

	arg_2_yaw.kp = 0.0f   *ANO_Param.PID_yaw.kp *0.001f;
	arg_2_yaw.ki = 0.0f   *ANO_Param.PID_yaw.ki *0.001f;
	arg_2_yaw.kd = 0.0f   *ANO_Param.PID_yaw.kd *0.001f;
...
void CTRL_1_PID_Init()
{
	...
	arg_1_yaw.kp = 2.0f   *ANO_Param.PID_yaw_s.kp *0.001f;
...
void CTRL_2(float dT,float weight,_copter_ctrl_st *data) //角度环
{
	PID_calculate(  dT,            			//周期
					...
					&arg_2_rol, 			//PID参数结构体
					&val_2_rol,				//PID数据结构体
					CTRL_2_INTER_LIMIT *weight,			//integration limit，积分限幅
					&(data->out_rol)  );	//输出
...
void CTRL_1(float dT,float weight,_copter_ctrl_st *data) //角速率环
{
	PID_calculate(  dT,            			//周期
					...
					&arg_1_rol, 			//PID参数结构体
					&val_1_rol,				//PID数据结构体
					CTRL_1_INTER_LIMIT *weight,			//integration limit，积分限幅
					&(data->out_rol)  );			//输出
...
=====================================================================================================================*/	
	...
	{	
		...
		//角度环控制
		CTRL_2(dT,gf.out_weight,&ctrl_2); //角度环
		//输出限幅
...
=====================================================================================================================*/	
	...
	ctrl_1.exp_yaw = my_deadzone(((0.002f *(+ch4))),0,0.1f) *MAX_YAW_SPEED;
	//限制最大期望角速度
	...
	//角速度环控制
	CTRL_1(dT,gf.out_weight,&ctrl_1); //角速度环
	//输出限幅
...
=====================================================================================================================*/	
	...
	//模拟反馈
	LPF_1(1,dT,exp_hspeed,&exp_hspeed_lpf);
...
{
	u8 i;
	motor[m1] = thr -ct_val_rol +ct_val_pit +ct_val_yaw;
	motor[m2] = thr +ct_val_rol +ct_val_pit -ct_val_yaw;
	motor[m3] = thr +ct_val_rol -ct_val_pit +ct_val_yaw;
	motor[m4] = thr -ct_val_rol -ct_val_pit -ct_val_yaw;
