...
//  USART_ClockInitTypeDef USART_ClockInitStruct;//异步方式不需要配置时钟--Leather
	...
	//GPIO_PinRemapConfig(GPIO_Remap_USART1,ENABLE);//不需要重映射到其它IO口
	...
	//配置USART1时钟--异步方式不需要配置时钟--Leather
...
void Uart1_IRQ(void)
{
	...
	{
		u8 com_data = USART1->DR;//读数据的同时清除中断标志位--Leather
