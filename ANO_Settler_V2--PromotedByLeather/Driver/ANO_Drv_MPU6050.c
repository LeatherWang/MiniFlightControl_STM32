...
void mpu_type(float x,float y,float z)//6050批次问题检测
{
	...
	{
		ANO_Param.sensor_type = MPU_6050_1;//枚举类型数据的用处！！！--Leather
	}
	else
	{
    ...
    if( acc_sum_cnt >= OFFSET_AV_NUM )
		{
			...
			ANO_Param_Save();//为什么只校验一次的原因所在！！！--Leather
...
//======================================================================
	/*陀螺仪转换到度每秒*/
	sensor.Gyro_deg.x = sensor.Gyro.x *0.0010636856f;//0.0610361f ;//  /65535 * 4000; +-2000度
	sensor.Gyro_deg.y = sensor.Gyro.y *0.0010636856f;//0.0610361f ;
	sensor.Gyro_deg.z = sensor.Gyro.z *0.0010636856f;//0.0610361f ;---------------leather
