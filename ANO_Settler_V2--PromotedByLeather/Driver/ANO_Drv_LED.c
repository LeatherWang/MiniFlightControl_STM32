...
void ANO_LED_Init(void)
{
	...
	GPIOA->BSRR = GPIO_Pin_5|GPIO_Pin_8;//off led
}
...
void led_breath(float dT,u16 T)// 一次半程的时间，单位ms
{
	...
	{
		case 0:
			on_time += safe_div(led_accuracy,(T/(dT*1000)));//20/(1500/20) 4/15 * 15*5(=1500/20=75) 半程时间1500ms
			if(on_time>20)
			{
				dir = 1;
			}
		break;
		case 1:
			on_time -= safe_div(led_accuracy,(T/(dT*1000)));
			if(on_time<0)
			{
				dir = 0;
			}
